...
function Row({ item, index }) {
  ...
  // Extract price and salePrice from weights array
  const price = item?.weights?.[0]?.price;  // Assuming weights array exists
  const salePrice = item?.weights?.[0]?.salePrice;  // Assuming salePrice is in weights as well
      ...
      <td className="border-y bg-white px-3 py-2 text-center">
        <div className="flex justify-center">
          <img
            ...
            src={item?.imageList?.[0]} // Assuming the first image in imageList
            alt={item?.title}
          />
        </div>
      </td>
